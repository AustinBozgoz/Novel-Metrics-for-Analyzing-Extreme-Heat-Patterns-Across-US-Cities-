...
def miami_PDF(TauNames,checkpoint='checkpoint.csv',months=[],years=[]):#tests if miami has been analyzed, and if not registers the city log info needed for the rest of the program
    TauColumns=['Tmax','Tavg','RHmax','RHavg','HImax','HIavg','WBTmax','WBTavg']#columns to be read
    masterFile=pd.read_csv(checkpoint)
    max99s,Q99s=[0]*8,[0]*8
    ...
    if masterFile[masterFile['dirNames']=='MIA second try']['graphs'].iloc[0]==0:#if it hasn't been analyzed yet
        Tau=tau_loader('MIA second try\\MIA daily maxes 1950 - 2019.csv',months=months,years=years)#open miami tau file
        ...
        for i in range(len(max99s)):
            log=pd.read_csv('city logs '+TauColumns[i]+' '+monthNames+' '+str(years[0])+'-'+str(years[1])+'.csv')
...
def PDF_progress(checkpoint='checkpoint.csv',update=False,city=None):#tracks progress of the graphs made or loads info on a select city
    ...
    if (update==False)&(city!=None):#if I'm calling a specific city
        ...
        if update==True:
            ...
            masterFile.to_csv(checkpoint)#save updated masterfile
    ...
    if cityDir=='MIA second try':
        stationID=''
    for root,dirs,files in os.walk(cityDir+'\\'+stationID):
...
def PDF_maker(Tau,months,years,TauName,miamiMax,cityName,stationID,folder,city=False,HI_overlay=False,record_city_logs=False):
    ...
    if record_city_logs==True:
        ...
        log=pd.read_csv('City logs\\city logs '+TauName+' '+monthNames+' '+yearNames+'.csv')
        if 'HI' in TauName:
        ...
        print(log)
        log.to_csv('City logs\\city logs '+TauName+' '+monthNames+' '+yearNames+'.csv')
    return

TauNames=['Tmax','Tavg','RHmax','RHavg','HImax','HIavg','WBTmax','WBTavg']
checkpoint='checkpoint.csv'

miamiMaxes=miami_PDF(TauNames=TauNames,months=months,years=years)#gather miami max99
if city!=False:#for a specific city
    file,cityDir,stationID=PDF_progress(city=city)#pick out city info
    Tau=tau_loader(file,months,years)#read file and store tau columns in tuple
    for i in range(len(TauNames)):
        PDF_maker(Tau,months,years,TauNames[i],miamiMaxes[i],cityDir,stationID,folder=cityDir+'\\'+stationID+'\\',city=city,HI_overlay=HI_overlay,record_city_logs=record_city_logs)
else:#for the full list
    stop=0
    update=False
    city=None
    while stop==0:
        file,cityDir,stationID=PDF_progress(update=update,city=city)
        Tau=tau_loader(file,months,years)#read file and store tau columns in tuple
        for i in range(len(TauNames)):
            PDF_maker(Tau,months,years,TauNames[i],miamiMaxes[i],cityDir,stationID,folder=cityDir+'\\'+stationID+'\\',city=False,HI_overlay=HI_overlay,record_city_logs=record_city_logs)
