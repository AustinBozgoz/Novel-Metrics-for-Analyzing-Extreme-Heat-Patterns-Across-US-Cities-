...
#------------controls----------------
directory=r'Augusta, ME\ASA LCD\\' #if I get the EOL complaint just play around with the backslashes
#note: if the first letter in the second folder is an N, just add a r to the front

first_year=1950
number_of_files=7
station='ASA' #must be string, must be 3 letter station name in file titles

period='annual' #which period is being examined? summer (JJA), summerext (JJASO) or annual
#-----------actual code--------------
...
def pd_index_backwards(database,column,status):#returns the last index in a column with a certain value (status)
    stop,i=0,0 #loop controls
    while stop==0: #search until target is found
        if i==len(database): #if target is not in list
            print('%s not present in %s[%s]'%(status,database,column)) #print error
            input()#wait for user to recognize
        elif database[column].iloc[-i-1]==status:stop=1 #target found
        else: i+=1#otherwise cycle entries
    return i #return index
def progress_controller_backwards(checkpoint='checkpoint.csv',update=False,city=None): #manages the checkpoint file and returns entries needed to operate the LCD file program
    ...
    nextIndex=pd_index_backwards(masterFile,'status',0) #find next city in checkpoint
    currentCity=masterFile['dirNames'].iloc[-nextIndex-1] #record the city name/1st directory
    stationID=masterFile['stationIDs'].iloc[-nextIndex-1]
    ...
    return currentCity,currentDir,stationID,fileNames,startYears,endYears
...
while stop==0:#never actually triggers
    currentCity,currentDir,currentStation,fileNames,startYears,endYears=progress_controller_backwards(update=update,city=city)#pull all variables needed to analyze this city
    ...
    ##analyze data
    currentProduct,RHavgall=LCD_analyze(currentDir,fileNames,endYears,currentProduct,RHavgall)#record info
    ...
    #save file
    currentProduct.to_csv(currentDir+currentStation+' daily maxes %s - %s.csv'%(startYears[0],endYears[-1]))
